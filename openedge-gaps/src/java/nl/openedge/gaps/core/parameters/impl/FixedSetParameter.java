...
/**
 * Parameter type voor een beperkte set geldige waarden, bijvoorbeeld te
 * renderen als een drop down box, list of een verzameling checkboxen.
 */
public final class FixedSetParameter extends Parameter {
    ...
    /**
     * Set met waarden voor snelle benadering - hoeven we niet door de inputs te
     * itereren.
     */
    ...
    /**
     * Construct.
     * @param validInputs array met invoer definities die als geldig worden
     *            aangemerkt
     * @param converter de optionele converter voor de invoer
     */
    public FixedSetParameter(ParameterInput[] validInputs, FixedSetInputConverter converter) {
        this();
     ...
     * @throws ValueOutOfRangeException indien de waarde NIET tot de toegestane
     *             set behoort
     */
    private void checkValueInSet(Object realValue) throws ValueOutOfRangeException {
        // check of de gegeven - geconverteerde waarde - tot de toegestane set
        // behoort
        if (!values.contains(realValue)) { throw new ValueOutOfRangeException(realValue
                + " behoort niet tot de set toegestane waarden"); }
    }

    /**
     * Shortcut naar parameter waarde.
     * @return instantie van geselecteerde object of null indien de waarde niet
     *         was gezet
     */
    ...
    private static final class Descriptor extends ParameterDescriptor {
        ...
        public Descriptor(Set inputs) {
            possibleValues = (ParameterInput[]) inputs.toArray(new ParameterInput[inputs.size()]);
        }
