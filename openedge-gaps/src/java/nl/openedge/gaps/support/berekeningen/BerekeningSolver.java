...
public class BerekeningSolver extends DepthFirstAdapter {
    ...
    public void outAPlusExp(APlusExp node) {
        ...
        if (log.isDebugEnabled()) {
            log.debug("op: " + x + " + " + y + " = " + result);
        }
        numberStack.push(new Double(result));
    }
    ...
    public void caseACallExp(ACallExp node) {
        ...
        try {
            ...
            for (Iterator i = parameters.iterator(); i.hasNext();) {
                ...
                if (log.isDebugEnabled()) {
                    log.info(functionName + " -> param: (" + param.getClass() + ") " + param
                            + " = " + parameterValue);
                }
            }
            ...
            if (value instanceof Number) {
                ...
                throw new BerekeningSolverException("resultaat functie " + functionName
                        + " is van een ongeldig type in deze context");
    ...
    public void outAIdExp(AIdExp node) {
        ...
        if (value != null) { // en zo ja, gebruik deze
            handleContextVariable(varName, value);
        } else { // geen context variabele geregisteerd; zoek in registry naar
            // parameter
    ...
    private void handleContextVariable(String name, Object value) {
        ...
        if (value instanceof Number) {
            ...
            throw new BerekeningSolverException("variabele " + name
                    + " is van een ongeldig type in deze context");
            // TODO ondersteuning voor bijv boolean
        }
    }
    ...
    public void handleParameter(String name) {
        ...
        try {
            Object result = gPathBrowser.navigate(expr);
            if (log.isDebugEnabled()) {
                log.debug("param: " + result);
            }
            if (result == null) { throw new BerekeningSolverException("expressie " + expr
                    + " gaf geen resultaten"); }
            if (result instanceof Number) {
                ...
                throw new BerekeningSolverException("objecten van het type "
                        ...
                        + " worden niet ondersteund in berekeningen (object = "
                        + result);
            }
