...
import nl.openedge.gaps.support.berekeningen.analysis.ReversedDepthFirstAdapter;
...
/**
 * Text display of the AST, with (optionally) color output. To print the AST we do a
 * reverse depth first traversal. We do this because it is easier to know which element is
 * the last child in any node. This makes it easier to do nice indenting.
...
public class ASTPrinter extends ReversedDepthFirstAdapter
{
	...
	/*
	 * As we visit each non-terminal node push on the indent we need for this node. The
	 * next node we visit will always be the last child of this node.
	 */
	...
	/*
	 * As we leave a non-terminal node, we pull off the indent character and prepend this
	 * nodes line to the output text.
	 */
	public void defaultOut(Node node)
	{
		// replace the current indent with the one from the stack
		indent = indent.substring(0, indent.length() - 3);
		indent = indent.substring(0, indent.length() - 1) + (String) indentchar.pop();
	...
	/*
	 * When we visit a terminals we just print it out. We always set last to false after
	 * this because the next node we visit will never be the last sibling.
	 */
	public void defaultCase(Node node)
	{
		...
		// prepend this line to the output
		output = indent
				+ "- " + setColor(BOLD, FG_GREEN, BG_BLACK) + ((Token) node).getText()
				+ treeColor() + "\n" + output;
	...
	/*
	 * A method to change the color codes. This only works on color-enabled terminals. In
	 * Windows/MS-DOS you need to load the ansi.sys driver from config.sys or
	 * c:\winnt\system32\config.nt (NT/win2k). ANSI.sys only works under Win2k in DOS
	 * mode. In UNIX, you need an ansi-enabled terminal...
	 */
