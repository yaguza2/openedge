...
/**
 * A Java class to determine image width, height and color depth for a number of image
 * file formats.
 * 
 * Based on ImageInfo written by Marco Schmidt <marcoschmidt@users.sourceforge.net>,
 * Version 1.3 <a
 * href="http://www.geocities.com/marcoschmidt.geo/image-info.html">original site</a>
 * 
 * Modified slightely to suit our (OpenEdge) needs (removed main method, put in package as
 * the original did not have a package specification)
 * 
 * Get file format, image resolution, number of bits per pixel and optionally number of
 * images, comments and physical resolution from JPEG, GIF, BMP, PCX, PNG, IFF, RAS, PBM,
 * PGM, PPM, PSD and SWF files (or input streams).
 * <p>
 ...
 * 	return;
 * }
 * System.out.println(ii.getFormatName() + &quot;, &quot; + ii.getMimeType() + &quot;, &quot; + ii.getWidth() + &quot; x &quot;
 * 	+ ii.getHeight() + &quot; pixels, &quot; + ii.getBitsPerPixel() + &quot; bits per pixel, &quot;
 * 	+ ii.getNumberOfImages() + &quot; image(s), &quot; + ii.getNumberOfComments() + &quot; comment(s).&quot;);
 * </pre>
 * 
 * You can also use this class as a command line program. Call it with a number of image
 ...
 * Known limitations:
 * <ul>
 * <li>When the determination of the number of images is turned off, GIF bits per pixel
 * are only read from the global header. For some GIFs, local palettes change this to a
 * typically larger value. To be certain to get the correct color depth, call
 * setDetermineImageNumber(true) before calling check(). The complete scan over the GIF
 * file will take additional time.</li>
 * <li>Transparency information is not included in the bits per pixel count. Actually, it
 * was my decision not to include those bits, so it's a feature! ;-)</li>
 * </ul>
 * <p>
 * Requirements:
 * <ul>
...
public final class ImageInfo
{
	...
	/**
	 * Return value of {@link #getFormat()} for JPEG streams. ImageInfo can extract
	 * physical resolution and comments from JPEGs (only from APP0 headers). Only one
	 * image can be stored in a file.
	 */
	public static final int FORMAT_JPEG = 0;

	/**
	 * Return value of {@link #getFormat()} for GIF streams. ImageInfo can extract
	 * comments from GIFs and count the number of images (GIFs with more than one image
	 * are animations). If you know of a place where GIFs store the physical resolution of
	 * an image, please <a
	...
	/**
	 * Return value of {@link #getFormat()} for PNG streams. PNG only supports one image
	 * per file. Both physical resolution and comments can be stored with PNG, but
	 * ImageInfo is currently not able to extract those.
	 */
	public static final int FORMAT_PNG = 2;

	/**
	 * Return value of {@link #getFormat()} for BMP streams. BMP only supports one image
	 * per file. BMP does not allow for comments. The physical resolution can be stored.
	...
	/**
	 * Return value of {@link #getFormat()} for PCX streams. PCX does not allow for
	 * comments or more than one image per file. However, the physical resolution can be
	 * stored.
	 */
	...
	/**
	 * Return value of {@link #getFormat()} for RAS streams. Sun Raster allows for one
	 * image per file only and is not able to store physical resolution or comments.
	 */
	...
	/**
	 * The names of all supported file formats. The FORMAT_xyz int constants can be used
	 * as index values for this array.
	 */
	private static final String[] FORMAT_NAMES = {"JPEG", "GIF", "PNG", "BMP", "PCX", "IFF", "RAS",
		"PBM", "PGM", "PPM", "PSD", "SWF"};

	/**
	 * The names of the MIME types for all supported file formats. The FORMAT_xyz int
	 * constants can be used as index values for this array.
	 */
	private static final String[] MIME_TYPE_STRINGS = {"image/jpeg", "image/gif", "image/png",
	 ...
	 * #setInput(InputStream)} or {@link #setInput(DataInput)}. If true is returned, the
	 * file format was known and information about its content can be retrieved using the
	 * various getXyz methods.
	 ...
	 *             if index is smaller than 0 or larger than or equal to the number of
	 *             comments retrieved
	 ...
	 * FORMAT_xyz constants from this class. Use {@link #getFormatName()} to get a textual
	...
	/**
	 * If {@link #check()} was successful, returns one the image's vertical resolution in
	 * pixels.
	 * 
	 ...
	 * called with <code>true</code> as argument, returns the number of comments retrieved
	 * from the input image stream / file. Any number &gt;= 0 and smaller than this number
	 * of comments is then a valid argument for the {@link #getComment(int)} method.
	 * 
	 ...
	 * {@link #check()}. This value can currently be only different from <code>1</code>
	 * for GIF images.
	 * 
	 ...
	 * (dots per inch) or -1 if no value could be found.
	 * 
	 * @return physical height (in dpi)
	...
	/**
	 * Returns the physical width of an image in inches, or <code>-1.0f</code> if width
	 * information is not available. Assumes that {@link #check} has been called
	 * successfully.
	 * 
	...
	/**
	 * If {@link #check()} was successful, returns one the image's horizontal resolution
	 * in pixels.
	 * 
	 ...
	 * is <code>false</code>. If enabled, comments will be added to an internal list.
	 * 
	 * @param newValue
	 ...
	 * <code>false</code>. This is a special option because some file formats require
	 * running over the entire file to find out the number of images, a rather
	 * time-consuming task. Not all file formats support more than one image. If this
	 * method is called with <code>true</code> as argument, the actual number of images
	 * can be queried via {@link #getNumberOfImages()} after a successful call to
	 * {@link #check()}.
	 * 
	 * @param newValue
