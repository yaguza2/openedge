 ...
 * Creates (what appears to be) a singleton controller object appropriate
 * for the type of controller specified in the XML, including single-use
 * controllers.  If no controller is specified, a special do-nothing
 * controller is returned. If you want to use a custom controller factory,
 * it is advisable to extend from this class and override either the
 * interface method (createController) or override one or more of the
 * template methods (getControllerClass, getControllerInstance and
...
public abstract class AbstractControllerFactory implements ControllerFactory
{
     ...
     * @see org.infohazard.maverick.flow.ControllerFactory#init(org.jdom.Element, javax.servlet.ServletConfig)
     */
	 ...
	 * @param controllerNode the xml node of the controller
	 * @return Class the class from the configuration or null if the controller node was null
	 * @throws ConfigException
	 */
	protected Class getControllerClass(Element controllerNode) throws ConfigException
	{
	    if(controllerNode == null) return null;

		String className = controllerNode.getAttributeValue(ATTR_CONTROLLER_CLASS);
		if (className == null || className.trim().equals(""))
		{
			throw new ConfigException(
			    "Controller element must have " + ATTR_CONTROLLER_CLASS
	...
	/**
	 * Create a controller instance (or a decorator) based on the controller class
	 * and the controller node.
	 ...
	 * @param controllerNode xml node of controller
	 ...
	 * @param controllerNode controller node
	 ...
	 */
	protected Controller createSingletonController(
	        Element controllerNode,
	        Class controllerClass)
			throws ConfigException,
	...
	/**
	 * Create a throwaway controller; this implementation creates
	 * an instance of ThrowawayControllerAdapter.
	 * @param controllerNode controller node
	 * @param controllerClass controller class
	 * @return Controller instance
	 * @throws ConfigException
	 ...
	 * @throws IllegalAccessException
	 */
	protected Controller createThrowawayController(
	        Element controllerNode,
	        Class controllerClass)
			throws ConfigException,
			InstantiationException,
	...
	/**
	 * initialize the controller if it needs to be done. This implementation
	 * just looks if the given controller (or embedded controller in case
	 * the given controller is a decorator) is of type ControllerSingleton and,
	 * if it is, it's init method is called.
	 * @param controllerNode the xml node of the controller
	 * @param controller the controller instance
	 * @throws ConfigException
	 */
	 ...
	 * @param controller the controller or wrapper.
