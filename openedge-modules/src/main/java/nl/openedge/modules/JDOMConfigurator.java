...
/**
 * <p>
 * Clients of the ComponentRepository should either construct the factory with an instance
 * of <code>javax.servlet.ServletContext</code> or with an instance of
 * <code>java.lang.String</code>. The first is for usage within a web application
 * environment and tries to read the location of the configuration document from
 * <code>javax.servlet.ServletContext.getInitParameter("oemodules.configFile")</code> If
 * you are not within a web application context, or if you want to load the configuration
 * from a location outside the web application environment, you can provide an URL as
 * either a string or an URL object, e.g: file://c:/mywinboxdrive/mydir/mycomponents.xml.
 * If you provide a string with a relative path, e.g: /mycomponents.xml, the configurator
 * tries to load from the classpath, where / is the classpath root. <br/>
