...
/**
 * Helper class for all transform steps. When extending this class, remember that you are
 * providing the TransformStep interface, so the getResponse(), etc methods are for a
 * *client* to call. You should be sending your output to the methods on getNext(), which
 * is safe to call multiple times.
 * 
 * Remember, the goal is to take data written to you and send it on to the next step!
 */
public abstract class AbstractTransformStep implements TransformStep
{
	...
	public HttpServletResponse getResponse() throws IOException, ServletException
	{
		...
		if (this.fakeResponse == null)
			this.fakeResponse =
				new FakeHttpServletResponse(this.getTransformCtx().getRealResponse());
