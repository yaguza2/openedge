...
import org.infohazard.maverick.flow.DefaultControllerFactory;
 ...
 * 
 *          <p>
 *          This factory has certain options which can be defined when declaring the
 *          factory. The defaults are shown here:
 *          </p>
 * 
 *          <pre>
 ...
 *          <ul>
 *          <li>
 *          <b>default-output-type</b> - Unless an individual transform explicitly
 *          specifies an output-type, this will be the http content-type for a
 *          successfully completing transformation.</li>
 *          <li>
 *          <b>template-caching</b> - Must be one of three values: <b>preload</b>, which
 *          compiles and caches all XSLT templates when the web application starts,
 *          <b>lazy</b>, which caches compiled XSLT templates but does not load them until
 *          they are requested, and <b>disabled</b>, which causes templates to be
 *          (re)loaded for every request. The default is preload.</li>
 *          <li>
 *          <b>uri-resolver</b> - The classname of a custom URI Resolver to set on all
 *          transformations.</li>
 *          </ul>
 * 
 *          <p>
...
public class XSLTransformFactory implements TransformFactory
{
	...
	/**
	 * If the init param for default content type is not specified, this becomes the
	 * actual default final content type. The default default :-)
	 */
	protected final static String DEFAULT_DEFAULT_FINAL_CONTENT_TYPE = "text/html";

	/**
	 * Unless specified in individual transform nodes, this is the content type for a
	...
	/**
	 * @param factoryNode
	 *            The XML from which to draw configuration information.
	 * @param servletCfg
	...
	public Transform createTransform(Element transformNode) throws ConfigException
	{
		...
		if (path == null)
			throw new ConfigException("XSLT transform node must have a \"" + ATTR_PATH
				+ "\" attribute:  " + XML.toString(transformNode));

		return new XSLTransform(path, isMonitored, this.templateCachingStyle, this.servletCtx,
			outputType, this.uriResolver);
	}
}
