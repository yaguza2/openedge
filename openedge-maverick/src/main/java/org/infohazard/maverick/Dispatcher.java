 ...
 * All commands are routed to this servlet by way of extension mapping
 * (say, *.m).  From here requests are routed through the "workflow"
 * tree of {@link Command}, {@link org.infohazard.maverick.flow.View View}, and
 * {@link org.infohazard.maverick.flow.Transform Transform} (or "Pipeline")
 * objects built from the Maverick configuration file.
 * </p>
 *
 * <p>
 * Commands can be gracefully chained together; if a view references
 * another Maverick Command, the same {@link MaverickContext} object is used.
 * </p>
 *
 * <p>
 * The Dispatcher object is made available to
 * {@link org.infohazard.maverick.flow.Controller Controllers} (or anyone else)
 * as an object in the application-scope (aka {@link ServletContext} attribute)
 * collection.
 * The attribute key is the value of the {@link #MAVERICK_APPLICATION_KEY
 * MAVERICK_APPLICATION_KEY} constant.
 * </p>
 *
 * <p>Note that there is are two special pseudocommands defined by this
 * servlet:  "<code>*</code>" and "<code>reload<code>".</p>
 *
 * <p>
 * "<code>reload</code>" triggers a reload of the maverick config file.
 ...
 * Note that the actual command name used for "<code>reload</code>" is
 * determined by the <code>reload</code> Servlet init parameter.
 * </p>
 *
 * <p>
 * "<code>*</code>" is a special command which can be defined in the
 * configuration file.
...
public class Dispatcher extends HttpServlet
{
     ...
     * the <code>Dispatcher</code> will be made available ["mav.dispatcher"].
     * </p>
	 */
	...
	/**
     * <p>
	 * If a value is set as an application attribute with this key,
	 * the value is used to override the setting of the <code>configFile</code>
	 ...
	 * Maverick configuration file ["configFile"].
     * This parameter is used when {@link #KEY_CONFIG_FILE} is not set,
     * otherwise the path defaults to
	 ...
	 * <p>
     * If a value is set as an application attribute with this key,
	 * the value is used to override the setting of the
     * <code>configTransform</code> Servlet init parameter
     * ["mav.configTransform"].
     * </p>
	 */
	...
	/**
     * <p>
	 * Name of the Servlet init parameter which defines the path to a
	 * transform which will be applied to the Maverick configuration
     * XML document before loading ["configTransform"].
	 * Defaults to null, which means perform no transformation.
     * </p>
	 */
	 ...
	 * <code>reload</code> Command ["reloadCommand"].
     * The value will typically be something like "reload".
     * </p>
	 */
	 ...
	 * <p>
     * Name of the Servlet init parameter which defines the name of the
	 * Command which displays the current configuration
     * ["currentConfigCommand"].
     * The value will typically be something like "currentConfig".
     * </p>
	 */
	public static final String INITPARAM_CURRENT_CONFIG_COMMAND =
            "currentConfigCommand";

	/**
	 * <p>
     * Name of the Serlvet init parameter used to set the
     * {@link #defaultRequestCharset defaultRequestCharset} property
     * ["defaultRequestCharset"].
     * </p>
	 */
	public static final String INITPARAM_DEFAULT_REQUEST_CHARSET =
            "defaultRequestCharset";

	/**
	 * <p>
     * Name of the Serlvet init parameter used to set the
     * {@link #limitTransformsParam limitTransformsParam} property
     * ["limitTransformsParam"].
     * </p>
	 */
	public static final String INITPARAM_LIMIT_TRANSFORMS_PARAM =
            "limitTransformsParam";

	/**
	 * <p>
     * Name of the Serlvet init parameter used to set the {@link
     * #reuseMaverickContext reuseMaverickContext} property
	...
	/**
     * <p>
	 * Default, context-relative, location of the Maverick XML configuration
     * file ["/WEB-INF/maverick.xml"].
     * </p>
     * <p>
     * Used if {@link #KEY_CONFIG_FILE} and {@link #INITPARAM_CONFIG_FILE} are
     * not set.
     * </p>
	 */
	...
	/**
	 * <p>
     * The {@link MaverickContext} object is stored in the request context with
	 * this key so that it can be recovered for recursive maverick execution
     * ["mav.context"].
     * </p>
	 */
     ...
     * [<code>null</code>].
     * If not set, the default charset will be whatever the servlet
     * container chooses (probably ISO-8859-1 aka Latin-1).
     * If set, this String is used as the character encoding for HTTP
	 * requests.
     * Leaving the property unset means do nothing special.
     * </p>
     * <p>
     * This property may be set through the
     * {@link #INITPARAM_DEFAULT_REQUEST_CHARSET
	...
	/**
	 * <p>
     * The number of transformations to run before stopping, regardless of
     * whether the final step has been reached.
     * If this property is not set, all transforms will run to completion.
     * </p>
     * <p>
     * This property may be set through the
     * {@link #INITPARAM_LIMIT_TRANSFORMS_PARAM
	...
	/**
     * <p>
     * If set to <code>true</code>, the {@link MaverickContext} is reused
     * between Commands invoked within the same request.
     * This allows Maverick Controllers to be "chained" by forwarding
     * context attributes from one Maverick Command to another.
     * <p>
     * The Context is <b>not</b> preserved in the case of a redirected
     * request, since redirection creates a new HTTP request.
     * </p>
     * <p>
     * This property may be set through the
     * {@link #INITPARAM_REUSE_CONTEXT INITPARAM_REUSE_CONTEXT} Serlvet init
     * parameter.
     * Set the parameter to "true" or leave it undefined ["false"].
     * </p>
	 */
            ...
            response) throws IOException, ServletException
	{
		...
		else
		{
			...
			// Maybe we want to use the same context object if we were recursively called from a
     ...
     * Extension and leading / will be removed.
     * </p>
	 */
     ...
     * Any requests being serviced are allowed to complete with the old data.
     * </p>
	 */
	protected void reloadConfig() throws ConfigException
	{
		...
		Loader loader = new Loader(replacementConfigDocument,
		...
		if (currentConfigStr !=	null)
		{
			Command	currentConfig =	new	Command() {
				public void	go(MaverickContext mctx) throws	IOException, ServletException
				{
					XMLOutputter outputter = new XMLOutputter(Format.getPrettyFormat());//("	 ",	true, "UTF-8");
	 ...
	 * If the command is not found, a command with name "*" is returned.
	 * If there is no command with id "*", null is returned.
     * </p>
	 */
	...
	protected Document loadConfigDocument() throws ConfigException
	{
		try
		{
			...
			String configTransform = (String)this.getServletContext().getAttribute(KEY_CONFIG_TRANSFORM);
			...
			else	// must perform a transformation
			{
				...
				try
				{
					Transformer transformer = TransformerFactory.newInstance()
						.newTransformer(new StreamSource(transURL.openStream(), transURL.toString()));
	...
	/**
     * <p>
	 * Returns the {@link #limitTransformsParam} property.
     * <code>null</code> null indicates the feature is disabled.
     * </p>
	 */
